...
int pl(F_T f_field[ROW][COL]){
    ...
    Action tmp = BEAM_SEARCH(f_field);//ビームサーチしてtmpに最善手を保存
		...
		if(combo!=tmp.maxcombo){return 200;}
...
double sub() {
	...
	for (i = 0; i < PROBLEM; i++) {//PROBLEM問解く
		...
		*/
		...
		if(combo!=tmp.maxcombo){mistake++;student+=200;}
		else{acc++;student+=(double)path_length;}
		printf("mistake=%d\n",mistake);
		printf("acc=%d\n",acc);
        ...
        printf("student=%lf,teacher=%lf\n",student,teacher);
		printf("------------\n");
		avg += (double)combo;
		oti_avg += (double)oti;
	}
  if(abs(teacher)<0.1){teacher=1.0;}
