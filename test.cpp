...
emilib::HashMap<int, bool> visited[ROW*COL];
emilib::HashMap<int, bool> visited2;

void dfs(F_T field[ROW][COL],int pos,int prev){
int hash=0;
for(int row=0;row<ROW;row++){
for(int col=0;col<COL;col++){
if(field[row][col]==(F_T)4){
hash+=poww[(row*COL)+col];
}
}
}

if(visited[pos][hash]){return;}
visited[pos][hash]=true;
if(!visited2[hash]){visited2[hash]=true;ans++;}
...
int nx=x+dx[dir];
int ny=y+dy[dir];
if((0<=x&&x<COL)&&(0<=y&&y<ROW)&&(0<=nx&&nx<COL)&&(0<=ny&&ny<ROW)&&(prev+dir!=3)){//1
swap(field[y][x],field[ny][nx]);
dfs(field,(ny*COL)+nx,dir);
swap(field[y][x],field[ny][nx]);  
}//1
}//2
