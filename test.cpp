...
int evaluate2(F_T field[ROW][COL], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot);//落とし減点評価関数
int sum_e2(F_T field[ROW][COL], sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot);//評価関数

ll xor128();//xorshift整数乱数
ll zoblish_field[ROW][COL][DROP+1];

ll sqBB[64];
int evaluate3(ll dropBB[DROP+1], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot);//落とし減点評価関数
int sum_e3(ll dropBB[DROP+1], sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot);//評価関数
...
ll ha;
bool ot;
cand.prev_score=sum_e2(ff_field,&cmb,&ha,p_maxcombo,&ot);
cand.improving=0;
cand.hash=ha;
cand.ot=ot;
dque.push_back(cand);
...
ll ha;
bool ot;
ca.prev_score=sum_e2(ff_field,&cmb,&ha,p_maxcombo,&ot);
ca.improving=0;
ca.hash=ha;
ca.ot=ot;
dque.push_back(ca);
...
ll ha;
bool ot;
cand.score = sum_e2(field, &cmb,&ha,p_maxcombo,&ot);
cand.combo = cmb;
cand.hash=ha;
cand.ot=ot;
...
int evaluate2(F_T field[ROW][COL], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot) {
int ev = 0;
*combo = 0;
ll ha=0;
int oti = 0;
...
ev += oti;
*hash=ha;
if(field[3][1]==4&&field[3][2]==4&&field[4][1]==6&&field[4][2]==6&&field[4][4]==3&&field[4][5]==3){*ot=true;}
return ev;
}
int evaluate3(ll dropBB[DROP+1], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot) {
int ev = 0;
*combo = 0;
ll ha=0;
int oti = 0;
...
int sum_e3(ll dropBB[DROP+1], sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot) {//落とし有り、落ちコン無し評価関数
return evaluate3(dropBB, EVAL_FALL | EVAL_COMBO, combo,hash,p_maxcombo,ot);
}
int sum_e2(F_T field[ROW][COL], sc* combo, ll* hash,int p_maxcombo[DROP+1],bool* ot) {//落とし有り、落ちコン無し評価関数
return evaluate2(field, EVAL_FALL | EVAL_COMBO, combo,hash,p_maxcombo,ot);
}
int sum_e(F_T field[ROW][COL]) {//落とし有り、落ちコン無しコンボ数判定関数
