...
//コンパイル
Windows10:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread test.cpp loguru.cpp -o test -mcmodel=large
Linux:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread -ldl test.cpp loguru.cpp -o test -mcmodel=large
...
201053210251533425501353123221

：26手以下(p)//9combo,飲み薬

015315151020442313510540210411

：27手以下(p)//9combo,nanashi

432015152244350331552132312515

：31手以下(p)//ore,9combo

323243441332042002331313014300

：19手以下(p)//ririoji,8combo

225530333313140355004550251403

：24手以下(p)//ja_mmk,9combo

224234425402054400304510125043

：30手以下(p)//nakaipad,8combo

053241405407470557104053134522

：41手以下(p)//ririoji,10combo

444404400004444404400004400004

：15手以下(p),9combo

000222044442044442044442000222

：29手以下(p),10combo

414102130144124234333230002120

：24手以下(p),10combo

404405055004404054405555504005

：14手以下(p),9combo


045004045554454404455004450050

：8手以下(p),9combo


354142404424442123102424035445
...
int ret=(int)s.size()-(int)top.size()-1;
...
#pragma omp parallel for
		...
		for (int j = 0; j < 4 * ks; j++) {
			...
			vec[fff[j].score+(BONUS*fff[j].improving)+(fff[j].nowR*3)+300].push_front(j);
			ks2++;
			}
		}
		part2+=omp_get_wtime() - start;
...
string BEAM_SEARCH2(F_T field[ROW][COL],int step,int MAX_TRN,int prev_dir,int now_pos); //ルート探索関数
string BEAM_SEARCH2(F_T field[ROW][COL],int step,int MAX_TRN,int prev_dir,int now_pos) {
...
*/
	...
	node2 ca;
	F_T board[ROW][COL]={0};
	memcpy(ca.field,field,sizeof(board));
	ca.pos=now_pos;
	if(step==0){
	ca.prev = -1;
	}
	...
	ac=BEAM_SEARCH(field,2,TRN,ca.prev,now_pos);
	ca.first_te = (T_T)YX(now_pos/COL,now_pos%COL);
	for (int trn = 0; trn <= TRN/21; trn++) {
	ca.movei[trn] = ac.moving[trn];
	}
	ca.calc_path();
	ca.calc_hash();
	ca.true_path+=to_string(now_pos%COL)+to_string((now_pos/COL)+5)+",";
	dque.push_back(ca);
	...
	//printf("depth=%d/%d\n",i+1,MAX_TRN);
	dque.clear();
	deque<int>vec[(ROW*COL/3)+1][1001];
	for(int j=0;j<4*ks;j++){
	...
	vec[combo][ff[j].path_length].push_front(j);
	}
	}
	int push_node=0;
	int possible_score;
	for(int k=(ROW*COL/3);k>=0;k--){
	possible_score=0;
	for (int j = 0; push_node < BEAM_WIDTH2 ;j++) {
	if(possible_score>1000){break;}
	if((int)vec[k][possible_score].size()==0){
	possible_score++;
	continue;
	}
	int v=vec[k][possible_score][0];
	node2 temp = ff[v];
	//swap(vec[possible_score][0], vec[possible_score].back());
	//vec[possible_score].pop_back();
	vec[k][possible_score].pop_front();
	F_T f_field[ROW][COL];
...
*/
	...
	for (int i = 0; i < ROW; i++) {
	for (int j = 0; j < COL; j++) {
	string tmp = BEAM_SEARCH2(field,0,TRN,-1,(i*COL)+j);
	node2 cand;
	F_T f_field[ROW][COL];
	...
	cand.prev = j;
	string tmp = BEAM_SEARCH2(f_field,i+2,TRN,cand.prev,cand.pos);
	memcpy(cand.field,f_field,sizeof(f_field));
	cand.path_length=calc_path_length(tmp);
	cand.calc_hash();
	//cout<<"path_length="<<cand.path_length<<endl;
