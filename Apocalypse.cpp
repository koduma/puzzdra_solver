...
//Apocalypse.cppをダウンロード
wget --no-check-certificate https://raw.githubusercontent.com/koduma/puzzdra_solver/master/Apocalypse.cpp
...
//コンパイル
Linux:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread Apocalypse.cpp loguru.cpp -o Apocalypse -mcmodel=large -ldl
Windows10,Windows11:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread Apocalypse.cpp loguru.cpp -o Apocalypse -mcmodel=large
MacOS:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread Apocalypse.cpp loguru.cpp -o Apocalypse -ldl
...
double evaluate2(F_T field[ROW][COL], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1]);//落とし減点評価関数
double sum_e2(F_T field[ROW][COL], sc* combo, ll* hash,int p_maxcombo[DROP+1]);//評価関数
...
ll sqBB[64];
double evaluate3(ll dropBB[DROP+1], int flag, sc* combo, int p_maxcombo[DROP+1]);//落とし減点評価関数
double sum_e3(ll dropBB[DROP+1], sc* combo, int p_maxcombo[DROP+1]);//評価関数
...
#pragma omp parallel for
		...
		for (int j = 0; j < 4 * ks; j++) {
			...
			double sco=fff[j].score+(double)((BONUS*fff[j].improving)+(fff[j].nowR*3));
			pque.push(make_pair(sco,j));
			ks2++;
			}
		}
		part2+=omp_get_wtime() - start;
		...
		for (int j = 0; push_node < BEAM_WIDTH ;j++) {
			if(pque.empty()){break;}
			int v=pque.top().second;pque.pop();
			node temp = fff[v];
...
double evaluate2(F_T field[ROW][COL], int flag, sc* combo, ll* hash,int p_maxcombo[DROP+1]) {
	double ev = 0;
	*combo = 0;
	ll ha=0;
	int oti = 0;
	...
	while (1) {
		...
		double ccc=(double)((p_maxcombo[i])*(p_maxcombo[i]));
		double vvv=(double)((d_maxcombo[i])*(d_maxcombo[i]));
		cmb2-=(double)(right[i]-left[i])*sqrt(1.0-(vvv/ccc));
		}
		}
...
double evaluate3(ll dropBB[DROP+1], int flag, sc* combo, int p_maxcombo[DROP+1]) {
	double ev = 0;
	*combo = 0;
	int oti = 0;
	ll occBB=0;
	...
	while (1) {
		...
		double ccc=(double)((p_maxcombo[i])*(p_maxcombo[i]));
		double vvv=(double)((d_maxcombo[i])*(d_maxcombo[i]));
		cmb2-=(double)(LSB-MSB)*sqrt(1.0-(vvv/ccc));
		}
...
double sum_e3(ll dropBB[DROP+1], sc* combo, int p_maxcombo[DROP+1]) {//落とし有り、落ちコン無し評価関数
	return evaluate3(dropBB, EVAL_FALL | EVAL_COMBO, combo,p_maxcombo);
}
double sum_e2(F_T field[ROW][COL], sc* combo, ll* hash,int p_maxcombo[DROP+1]) {//落とし有り、落ちコン無し評価関数
