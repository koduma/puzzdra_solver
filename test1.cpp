...
//test1.cppをダウンロード
wget --no-check-certificate https://raw.githubusercontent.com/koduma/puzzdra_solver/master/test1.cpp
...
//コンパイル
Linux:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread test1.cpp loguru.cpp -o test1 -mcmodel=large -ldl
Windows10,Windows11:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread test1.cpp loguru.cpp -o test1 -mcmodel=large
MacOS:g++ -std=c++11 -fopenmp -mbmi2 -lpthread test1.cpp loguru.cpp -o test1 -ldl
...
int calc_ev(string str,F_T field[ROW][COL]){
...
*/
  ...
  improve=false;  
	...
	cand.pos=(i*COL)+j;
	cand.prev=-1;
	memcpy(cand.field,field,sizeof(cand.field));
	cand.true_path=to_string(j)+to_string(i+5)+",";
	if(improve){
	cand.path_length=predict;
	}
	else{
	cand.path_length=pl;
	}
