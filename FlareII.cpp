...
//FlareII.cppをダウンロード
wget --no-check-certificate https://raw.githubusercontent.com/koduma/puzzdra_solver/master/FlareII.cpp
...
//コンパイル
Linux:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread FlareII.cpp loguru.cpp -o FlareII -mcmodel=large -ldl
Windows10,Windows11:g++ -O2 -std=c++11 -fopenmp -mbmi2 -lpthread FlareII.cpp loguru.cpp -o FlareII -mcmodel=large
MacOS:g++ -std=c++11 -fopenmp -mbmi2 -lpthread FlareII.cpp loguru.cpp -o FlareII -ldl
...
#define BEAM_WIDTH 28000//MAX2800000
...
bool prun(double b,double t,double ev,double stop,double plan){
double x=((stop-b)/(ev-b))*t;
if(x<0||x>(double)TRN){x=(double)TRN;}
if(x>=plan){return true;}
return false;
}
...
Action BEAM_SEARCH(F_T f_field[ROW][COL],int maxi,int MAX_TRN,int prev_dir,int now_pos,int stop) {
	...
	double b=(double)dque[0].prev_score;
...
#pragma omp parallel for
		...
		for (int j = 0; push_node < BEAM_WIDTH ;j++) {
			...
			if(i>=MAX_TRN/2&&push_node==0){if(prun(b,(double)i,(double)possible_score-2000,(double)stop*120,(double)MAX_TRN)){return bestAction;}}
