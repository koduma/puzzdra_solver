...
#define BEAM_WIDTH 20000//ビーム幅//MAX200000
...
Action BEAM_SEARCH(F_T f_field[ROW][COL],int stop,int basyo); //ルート探索関数
double part1 = 0, part2 = 0, part3 = 0, part4 = 0, MAXCOMBO = 0;
Action BEAM_SEARCH(F_T f_field[ROW][COL],int stop,int basyo) {
    ...
    cand.nowC=basyo%COL;
    cand.prev=-1;
    cand.first_te=(T_T)YX(basyo/COL,basyo%COL);
    F_T ff_field[ROW][COL];
	memcpy(ff_field,f_field,sizeof(ff_field));
	sc cmb;
	ll ha;
			...
			for (int j = 0; j < DIR; j++) {//上下左右の4方向が発生
				...
				if (0 <= cand.nowC + dx[j] && cand.nowC + dx[j] < COL &&
					...
					if (cand.prev + j != 3) {
						...
						cand.hash^=(zoblish_field[(int)cand.nowR][(int)cand.nowC][(int)tmp])^(zoblish_field[ny][nx][(int)field[ny][nx]]);
						cand.hash^=(zoblish_field[(int)cand.nowR][(int)cand.nowC][(int)field[ny][nx]])^(zoblish_field[ny][nx][(int)tmp]);
	...
	for (i = 0; i < 9; i++) {//PROBLEM問解く
		...
		for (j = 0; j < ROW; j++) {
			for (k = 0; k < COL; k++) {
                ...
                if(1<=f_field[j][k]&&f_field[j][k]<=DROP);
                else{cout<<"-1"<<endl;return 0;}
			}
		}
        }
		Action tmp=BEAM_SEARCH(f_field,i+1,basyo);//ビームサーチしてtmpに最善手を保存
		string layout="";		
		for(int v=0;v<ROW;v++){
		for(int u=0;u<COL;u++){
        ...
        basyo=XX(tmp.first_te)+YY(tmp.first_te)*COL;
		for (j = 0; j <= TRN/21; j++) {//y座標は下にいくほど大きくなる
			if (tmp.moving[j] == 0ll) { break; }
			for(k=0;k<21;k++){
			int dir = (int)(7ll&(tmp.moving[j]>>(3*k)));
			if (dir==0){break;}
			if (dir==1) { route+=to_string(3);swap(f_field[basyo/COL][basyo%COL],f_field[basyo/COL][(basyo%COL)-1]);basyo--;}//printf("L"); } //"LEFT"); }
			if (dir==2) { route+=to_string(6);swap(f_field[basyo/COL][basyo%COL],f_field[(basyo/COL)-1][basyo%COL]);basyo-=COL;}//printf("U"); } //"UP"); }
			if (dir==3) { route+=to_string(1);swap(f_field[basyo/COL][basyo%COL],f_field[(basyo/COL)+1][basyo%COL]);basyo+=COL;}//printf("D"); } //"DOWN"); }
			if (dir==4) { route+=to_string(4);swap(f_field[basyo/COL][basyo%COL],f_field[basyo/COL][(basyo%COL)+1]);basyo++;}//printf("R"); } //"RIGHT"); }
			}
		}
		cout<<route<<endl;
